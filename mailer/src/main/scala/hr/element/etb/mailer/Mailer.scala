...
trait Mailer extends SimpleInjector {
  ...
  final case class XHTMLMailBodyType(text: String) extends MailBodyType
  final case class XHTMLPlusImages(text: String, items: PlusImageHolder*) extends MailBodyType
  ...
  implicit def xmlToMailBodyType(html: NodeSeq): MailBodyType = XHTMLMailBodyType(html.toString())
